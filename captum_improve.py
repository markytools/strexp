...
### Function that runs all XAI attribution methods and have the last one
### combine LIME, Shapley, FeatAblt
def combineBestDataXAI(opt):
    ### targetDataset - one dataset only, CUTE80 has 288 samples
    targetDataset = "IC03_867" # ['IIIT5k_3000', 'SVT', 'IC03_867', 'IC13_1015', 'IC15_2077', 'SVTP', 'CUTE80']
    segmRootDir = "/home/goo/str/datasets/segmentations/224X224/{}/".format(targetDataset)
    outputSelectivityPkl = "metrics_sensitivity_eval_results_{}.pkl".format(targetDataset)
    ...
    ### Adjusted STRScore with char-contrib modification
    ...
    for i, (orig_img_tensors, segAndLabels) in enumerate(evaluation_loader):
        ...
        baseline_dist = torch.zeros((1, 1, opt.imgW, opt.imgH))
        ...
        topAttributions.append(attributions)
        if acquireSelectivity:
            n_correct, confidenceList = acquireSelectivityHit(img1, attributions, segmDataNP, model, converter, labels, scoring)
        ...
        ### Shapley Value Sampling with Char Contrib
        svs2 = ShapleyValueSampling(super_pixel_model_charContrib)
        # attr = svs.attribute(input, target=0, n_samples=200) ### Individual pixels, too long to calculate
        attributions = svs2.attribute(input, target=0, feature_mask=segmTensor)
        if acquireSelectivity:
            n_correct, confidenceList = acquireSelectivityHit(img1, attributions, segmDataNP, model, converter, labels, scoring)
            results_dict["shapley_charcontrib_acc"] = n_correct
            results_dict["shapley_charcontrib_conf"] = confidenceList
        if acquireInfidelity:
            infid = float(infidelity(super_pixel_model_charContrib, perturb_fn, img1, attributions).detach().cpu().numpy())
            results_dict["shapley_charcontrib_infid"] = infid
        if acquireSensitivity:
            sens = float(sensitivity_max(svs2.attribute, img1, target=0).detach().cpu().numpy())
            results_dict["shapley_charcontrib_sens"] = sens
        ...
        topAttrCombined = torch.mean(torch.cat(topAttributions, dim=1), dim=1).unsqueeze(0)
        # topAttrCombined, _ = torch.max(torch.cat(topAttributions, dim=1), dim=1)
        # topAttrCombined = topAttrCombined.unsqueeze(0)
        if acquireSelectivity:
            n_correct, confidenceList = acquireSelectivityHit(img1, topAttrCombined, segmDataNP, model, converter, labels, scoring)
            results_dict["combined_acc"] = n_correct
            results_dict["combined_conf"] = confidenceList
        # print("topAttrCombined shape: ", topAttrCombined.shape)
        # sys.exit()
