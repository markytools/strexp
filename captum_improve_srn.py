...
from utils import CTCLabelConverter, AttnLabelConverter, Averager, TokenLabelConverter, SRNConverter
import string
import time
import sys
from dataset import hierarchical_dataset, AlignCollate
import validators
from model_srn import Model, STRScore
...
def acquireSingleCharAttrAve(opt):
    ...
    length_for_pred = torch.cuda.IntTensor([opt.batch_max_length] * opt.batch_size)
    ...
    for i, (orig_img_tensors, segAndLabels) in enumerate(evaluation_loader):
        ...
        if settings.MODEL == 'vitstr':
            ...
            charOffset = 0 # SRN has no 'GO' token
        ...
        for charIdx in range(0, len(labels)):
            ...
            gtClassNum = target[0][0][charIdx + charOffset]
        ...
        channelDim = 3 if opt.rgb else 1
        baseline_dist = torch.zeros((1, channelDim, opt.imgH, opt.imgW))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--selective_sample_str', type=str, default='', help='If =='', only sample images with string matching this (see --sensitive for case sensitivity)')
